...
@Component({
  selector: 'app-city-close-tickets',
  templateUrl: './city-close-tickets.page.html',
  styleUrls: ['./city-close-tickets.page.scss'],
})
export class CityCloseTicketsPage implements OnInit {
  showForm = false;
...
if(window){
 ...
 this.ngxService.start();
  ...
  calculateResult(): number {
    const number1 = this.calculationForm.get('number1').value;
    const number2 = this.calculationForm.get('number2').value;
    const number3 = this.calculationForm.get('number3').value;
    const number4 = this.calculationForm.get('number4').value;

    // Perform your calculations here
    return number1 * number2 + number3 + number4 
  }
  calculate(): number {
    const number11 = this.calculation.get('number11').value;
    const number12 = this.calculation.get('number12').value;
    const number13 = this.calculation.get('number13').value;
    const number14 = this.calculation.get('number14').value;
  ...
  Add_cost() {
    if (window) {
      if(
        this.DataToSend.C_VisitorNo == "" || 
      this.DataToSend .C_VisitCharge == "" ||
      this.DataToSend.C_MaterialCost == "" ||
    this.DataToSend.C_LabourCost == "" ||

    this.DataToSend.T_VisitorNo == "" ||
    this.DataToSend.T_VisitCharge == "" ||
    this.DataToSend.T_MaterialCost == "" ||
