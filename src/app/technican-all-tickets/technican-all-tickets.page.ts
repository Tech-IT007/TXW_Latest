...
@Component({
  selector: 'app-technican-all-tickets',
  templateUrl: './technican-all-tickets.page.html',
  styleUrls: ['./technican-all-tickets.page.scss'],
})
export class TechnicanAllTicketsPage implements OnInit {
...
route_to_profile_answer_screen(userID: string) {
  // Store Booking ID
  ...
  if (
    ...
    this.router.navigate(['/branch-coporate-ticket-details'], {
      queryParams: { BookingID: this.BookingID }
    });

  // Technician / Vendor roles
  } else if (
    rolesArray.includes('Technician') ||
    rolesArray.includes('Vendor')
  ) {
    this.router.navigate(['/cop-current-tickets'], {
      queryParams: { BookingID: this.BookingID }
    });

  } else {
    console.warn('Role not authorized:', rolesArray);
  }
}
