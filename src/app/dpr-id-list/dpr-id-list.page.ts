...
@Component({
  selector: 'app-dpr-id-list',
  templateUrl: './dpr-id-list.page.html',
  styleUrls: ['./dpr-id-list.page.scss'],
})
export class DprIdListPage implements OnInit {
          ...
          if (response && response.data) {
            this.taskProgressList = response.data.map((item: any) => {
              ...
              return {
                ...
                showUpdate: this.isToday(item.TaskDate), // ✅ Only show if TaskDate is today
  ...
  // ✅ Show Update button only for today's date
  isToday(taskDate: string): boolean {
    ...
    try {
      ...
      return apiDate.getTime() === today.getTime(); // ✅ Only current date
    } catch {
      return false;
    }
  }
  ...
  showpicks() {
    const api = "https://techxpertindia.in/api/view_dpr_images.php";
    this.http.post(api, this.pick_user_id).subscribe((response) => {
